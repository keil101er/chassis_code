...
static void ui_update_text(ui_interface_string_t *obj, const char *text);
static void ui_update_overlay(void);
static void ui_refresh_static_text(void);
...
void test_task(void const *argument)
{
    ...
    static uint8_t ui_static_refresh_tick = 0;
    static uint8_t ui_text_repair_cycles = 0;
    ...
    while (1)
    {
        ...
        if (get_robot_id() != 0U)              
        {
            ...
            if ((ui_initialized == 0U) || (g_key_pressed != 0U && g_key_pressed_last == 0U))
            {
                ui_reinit_overlay();
                ui_initialized = 1;
                ui_static_refresh_tick = 0;
                ui_text_repair_cycles = 6U;
            }
            else
            {
                ui_update_overlay();
                if (ui_text_repair_cycles > 0U)
                {
                    ui_refresh_static_text();
                    ui_text_repair_cycles--;
                    ui_static_refresh_tick = 0;
                }
                else
                {
                    ui_static_refresh_tick++;
                    if (ui_static_refresh_tick >= 10U)
                    {
                        ui_refresh_static_text();
                        ui_static_refresh_tick = 0;
                    }
...
static void ui_update_overlay(void)
{
    ...
    leg_height = (int32_t)lroundf(40.0f + (leg_length - 0.13f) * 350.0f);
    leg_height = ui_clamp_i32(leg_height, 30, 110);
    angle_offset = (int32_t)lroundf(leg_angle * 40.0f);
    angle_offset = ui_clamp_i32(angle_offset, -30, 30);

    ui_g_Ungroup_leg_state->start_x = (uint32_t)(1633 + angle_offset);
    ui_g_Ungroup_leg_state->details_d = (uint32_t)(1637 + angle_offset);
    ui_g_Ungroup_leg_state->details_e = 382;
    ui_g_Ungroup_leg_state->start_y = (uint32_t)(382 - leg_height);
    ...
    show_follow = (uint8_t)((chassis_move_balance.start_flag != 0U) &&
                            (chassis_move_balance.w_flag == 0U) &&
                            (robot_state.power_management_chassis_output != 0U));
    show_weak = (uint8_t)((chassis_move_balance.start_flag == 0U) ||
                          (robot_state.power_management_chassis_output == 0U));

    ui_update_text(ui_g_Ungroup_fire_mode, show_fire ? "fric" : "");
    ui_update_text(ui_g_Ungroup_w_flag, show_gyro ? "gyro" : "");
    ui_update_text(ui_g_Ungroup_mode_rc_1, show_follow ? "follow" : "");
    ui_update_text(ui_g_Ungroup_mode_rc_0, show_weak ? "weak" : "");
