...
float turn_L_compensation=0.0f;
extern float v_l;
extern float v_r;



extern uint8_t AUTO_jump_flag;
...
void ChassisL_task(void)
{
	...
	PID_init(&jump_pid_L, PID_POSITION, jump_pid, 80.0f, 0.0f);
	// shoot init
	//  射击初始化
	shoot_init();
	...
	while (1)
	{
		...
		//
		//       CAN_cmd_gimbal(0,0,shoot_can_set_current,0);  //拨弹盘电机控制
		//	   osDelay(CHASSL_TIME);
		//		}
	}
 }
...
void chassisL_feedback_update(chassis_t *chassis, vmc_leg_t *vmc, INS_t *ins)
{
	...
	chassis->yaw_motor_angle = motor_ecd_to_angle_change(chassis->motor_chassis[4].ecd, 2865); // 获取相对角度值，零点ecd=2865
...
void chassisL_control_loop(chassis_t *chassis, vmc_leg_t *vmcl, INS_t *ins, float *LQR_K, pid_type_def *leg)
{
	...
	chassis->wheel_motor[1].wheel_T = (LQR_K[0] * (vmcl->theta + theat_set) + LQR_K[1] * (vmcl->d_theta - 0.0f)
									   ...
									   //										+LQR_K[3]*(chassis->v_set-chassis->v)
									   // +LQR_K[2]*0.0f
									   + LQR_K[2] * ((chassis->x_set) - chassis->x_filter) + LQR_K[3] * ((chassis->v_set) - chassis->v_filter2) + LQR_K[4] * (chassis->myPithL - (-0.0f)) + LQR_K[5] * (chassis->myPithGyroL - 0.0f));

	vmcl->Tp = (LQR_K[6] * (vmcl->theta + theat_set) + LQR_K[7] * (vmcl->d_theta - 0.0f)
				//						+LQR_K[8]*(chassis->x_set-chassis->x)
				//						+LQR_K[9]*(chassis->v_set-chassis->v)
				// +LQR_K[8]*0.0f
				+ LQR_K[8] * ((chassis->x_set) - chassis->x_filter) + LQR_K[9] * (chassis->v_set - chassis->v_filter2) + LQR_K[10] * (chassis->myPithL - (-0.0f)) + LQR_K[11] * (chassis->myPithGyroL - 0.0f));
	...
	turn_L_compensation=(2*vmcl->L0+0.4f)*8.5f*(v_l*v_l-v_r*v_r)/6.912f;
	mySaturate(&turn_L_compensation,-5.0f,5.0f);
