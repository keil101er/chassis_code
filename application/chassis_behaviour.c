...
/**
  ...
  */
/**
  * @brief          通过逻辑判断，判断"chassis_behaviour_mode"成哪种模式
...
/**
  ...
  * @retval         none
  */
void chassis_behaviour_control_set(fp32 *vx_set, fp32 *turn_set,chassis_t *chassis_move_rc_to_vector)
{


	    if (vx_set == NULL || chassis_move_rc_to_vector == NULL)
    {
      return;
    }
        ...
        if (chassis_behaviour_mode == CHASSIS_ZERO_FORCE)
    {
			chassis_down_control(chassis_move_rc_to_vector);
    }
//    else if (chassis_behaviour_mode == CHASSIS_REMOTE)
//    {
//			chassis_remote_control(vx_set, turn_set, chassis_move_rc_to_vector);
//    }
}

 void chassis_down_control( chassis_t *chassis_move_state)
{
	
	chassis_move_state->start_flag=0;
...
/**
  ...
  * @retval         返回空
  */
//static void chassis_follow_chassis_yaw_control(fp32 *vx_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector);
...
/**
  * @brief          底盘无力的行为状态机下，底盘模式是raw，故而设定值会直接发送到can总线上故而将设定值都设置为0
  */
//static void chassis_zero_force_control(chassis_t *chassis_move_rc_to_vector)
//{
