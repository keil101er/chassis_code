...
extern void dm4310_fbdata1(Joint_Motor_t *motor, uint8_t *rx_data,uint32_t data_len);
...
extern void mit_WHEEL(CAN_HandleTypeDef *hcan, uint16_t motor_id, float pos, float vel,float kp, float kd, float torq);

extern void c_transmit_date(float yaw_ctrl, float pitch_ctrl,uint8_t mode,uint8_t reserve1, uint8_t reserve2);
