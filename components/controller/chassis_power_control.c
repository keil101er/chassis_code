...
//		
		...
		* chassis_power_control->wheel_motor[i].given_current + constant;			//加常量
...
//			fp32 inside = b * b - 4 * a * c;
			...
			fp32 b = toque_coefficient * abs(chassis_power_control->wheel_motor[i].speed_rpm);
			fp32 c = k1 * chassis_power_control->wheel_motor[i].speed_rpm * chassis_power_control->wheel_motor[i].speed_rpm - scaled_give_power[i] + constant;
			fp32 inside = b * b - 4 * a * c;		
			...
			else if (chassis_power_control->wheel_motor[i].given_current > 0) // Selection of the calculation formula according to the direction of the original moment
			{
				fp32 temp = (-b + sqrt(inside)) / (2 * a);
				if (temp > 15000)
				{
					chassis_power_control->wheel_motor[i].given_current = 14000;
				}
				else
					chassis_power_control->wheel_motor[i].given_current = temp;
			}
			else
			{
				fp32 temp = (-b - sqrt(inside)) / (2 * a);
